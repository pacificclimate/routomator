...
if (x == 0) {
  ...
  if (x != 0) {
    stop('Unable to create or switch into previously created mapset... something is up and needs to be looked into')
  }
}
...
setwd(code.dir)
print(getwd())
A <- system2(paste(file.path(code.dir, "make_rout.sh"), file.path(tempfiles, 'flow-acc-15.asc'), sep=' '))
